...
func GetFavorites(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err = db.Select(&favoriteUser, "SELECT p.product_id, p.name, p.description, p.price, p.image_url FROM product p JOIN favorites f ON p.product_id = f.product_id  WHERE user_id = $1", id)
		if err != nil {
