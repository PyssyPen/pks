...
func GetProducts(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := db.Select(&products, "SELECT * FROM product ORDER BY product_id")
		if err != nil {
...
func CreateProduct(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		query := `INSERT INTO Product (name, description, price, image_url) 
                  VALUES (:name, :description, :price, :image_url) RETURNING product_id`
