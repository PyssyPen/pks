...
class ApiService {
  ...
  void updateProduct(Tovar product, int id) async {
    await _dio.put('http://localhost:8080/products/$id',
        data: product.toJson());
  }
  ...
  Future<List<Tovar>> getFavoriteProducts(int userId) async {
    try {
      final response =
          await _dio.get('http://localhost:8080/favorites/$userId');
      if (response.statusCode == 200) {
  ...
  void addProductToFavorite(Tovar product) async {
    await _dio.post('http://localhost:8080/favorites/1',
        data: product.toJson());
  }
  ...
  void updateCartProductPlus(CartModel cartItem, int id) async {
    await _dio.put('http://localhost:8080/carts/1/$id/plus',
        data: cartItem.toJson());
  }

  void updateCartProductMinus(CartModel cartItem, int id) async {
    await _dio.put('http://localhost:8080/carts/1/$id/minus',
        data: cartItem.toJson());
  }
